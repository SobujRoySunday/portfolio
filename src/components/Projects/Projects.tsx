...
const Projects = ({ projects }: { projects: Project[] }) => {
  return (
    <>
      <h2 className="p-8 uppercase text-[96px] lg:text-[192px]">Projects Showcase</h2>
      {projects && projects.map((project, index) => (
        <div key={project.id} className={`w-fit p-8 flex flex-col justify-center gap-4 ${index % 2 === 0 ? 'self-start' : 'self-start lg:self-end'}`}>
          <Image width={1920} height={1080} src={project.image} alt={project.name} className="object-cover w-auto h-[300px] lg:h-[600px] hover:-translate-y-5 duration-300" priority />
